...
export class FSModelConfigRepo implements IModelConfigRepo {
    ...
    async setConfig(config: z.infer<typeof ModelConfig>): Promise<void> {
        ...
        existingProviders[config.provider.flavor] = {
            ...
            awsRegion: config.provider.awsRegion,
            awsAccessKeyId: config.provider.awsAccessKeyId,
            awsSecretAccessKey: config.provider.awsSecretAccessKey,
            awsSessionToken: config.provider.awsSessionToken,
            model: config.model,
